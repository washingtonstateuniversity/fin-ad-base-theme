...
jQuery( document ).ready( function( $ ) {
	...
	$( '.multi-color-control' ).each( function() {
		...
		if( $controlTriggerWrap.find(".grouplist").length == 0 ){
			$controlTriggerWrap.html("<ul class='grouplist'></ul>");
		}
		...
		$_trigger.appendTo( $controlTriggerWrap.find(".grouplist") ).wrap( '<li class="multi-color-picker-trigger"></li>' );
		$("<span>"+colorTitle+"</span>").insertAfter($_trigger);
		$container.append($("<span class='close'>"));

		$(".wp-picker-container .close").off().on( 'click', function() {
			...
			console.log("closing things down");
				...
				$controlWrap.find( '.multi-color-control-color-title' ).html( '' );
		});
		...
		$_trigger.closest(".multi-color-picker-trigger").on( 'click', function() {
			...
			if ( $trigger.hasClass( 'wp-picker-open' ) ) {
				...
				$container.find( '.multi-color-control-color-title' ).html( '' );
				...
				$container.find( '.multi-color-control-color-title' ).html( "<strong>"+colorTitle+"</strong>" );
